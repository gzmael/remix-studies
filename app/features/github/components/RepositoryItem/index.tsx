...
export const RepositoryItem = ({ repo, user }: IRepositoryItemProps) => {

  const handleVisitSite = (e:React.MouseEvent<HTMLButtonElement, MouseEvent>, site: string) => {
  ...
  return (
    ...
    >
      <Link 
        prefetch="intent"
        to={`/github/${user.login}/${repo.name}`} 
      ...
      >
        ...
        <div className="hidden sm:flex flex-col flex-shrink-0 items-end space-y-3">
          <p className="flex items-center space-x-4">
            <button
              onClick={(e) => handleVisitSite(e, repo.html_url)}
            ...
            >
              <div className="flex items-center">
                <StarIcon
                  className="h-5 w-5 text-yellow-300 hover:text-yellow-400 mr-1"
                  aria-hidden="true"
                />
